  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <b-image
            class="position-little-top"
            ratio="4by3"
            :src="`${list.image}?width=200&height=150`"
            v-bind="list"
            src-fallback="http://placehold.it/200x150"
          >
...
import { sign, mod, bound } from '~/node_modules/buefy/src/utils/helpers'
import Icon from '~/node_modules/buefy/src/components/icon/Icon'
...
export default {
  ...
  watch: {
    ...
    value(value) {
      this.switchTo(
        this.asIndicator ? value - (this.itemsToShow - 3) / 2 : value,
      )
  ...
  methods: {
    ...
    checkAsIndicator(value, event) {
      ...
      const dragEndX = event.changedTouches
        ? event.changedTouches[0].clientX
        : event.clientX
      if (this.hold - Date.now() > 2000 || Math.abs(this.dragX - dragEndX) > 10)
        return
